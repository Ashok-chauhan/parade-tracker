const manageRoute = async () => {
	...
	content.forEach((element) => {
		//toRoute.innerHTML += `<li draggable="true" data-id="${element.id}"><strong>${element.intersection} </strong> LAT:  ${element.latitude}  LON:  ${element.longitude} <span>Edit</span> <span>delete</span></li>`;
		toRoute.innerHTML += `<tbody draggable="true" data-id="${element.id}"><tr  class="route-tr"><td class="route-name"> &#8593; &#8595; <strong>${element.intersection} </strong></td>   <td class="route-delete"><button class="btn-delete" onClick="deleteRoute(${element.id});">delete</button></td> <td class="route-edit" onClick="editRoute(${element.id});"><button class="btn-edit">Edit</button></td></tr></tbody>`;
	});
